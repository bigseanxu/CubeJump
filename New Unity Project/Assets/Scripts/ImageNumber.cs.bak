...
public class ImageNumber : MonoBehaviour {
	...
	public string Path = "ScoreNumber";
	Object [] sprites = new Object[10];
	...
	void Start () {
		sprites = Resources.LoadAll (Path);
	}
	...
	void GenerateImageNumber(int n) {
		print ("n = " + n);
		for (int i = 0; i < n; i++) {
			GameObject number = (GameObject) Instantiate (gameObject, new Vector3 (), new Quaternion());
	...
	void NumberToImage() {
		...
		for (int i = numberCount - 1; i >= 0; i--) {
			...
			gameObject.GetComponent<Image>().sprite = (Sprite)sprites[n];
			number /= 10;
		}
	}
